...
void main(void) {
    ...
    vec4 vertexPos = modelViewMatrix * vec4(aVertexPosition, 1.0);
    vPosition = vertexPos.xyz;
    // Transform normal by precomputed normal matrix
...
void main(void) {
    ...
    // Blinn-Phong specular: use halfway vector
    vec3 halfVector = normalize(lightDir + viewDir);
    float NdotH = max(dot(normal, halfVector), 0.0);
    float spec = pow(NdotH, 64.0); // Smoother highlights
    spec *= step(0.0, NdotL);      // Remove specular on back-facing surfaces

    // Energy-conserved lighting model
    vec3 diffuse = halfLambert * uLightColor;
    vec3 specular = spec * uLightColor * 0.4; // slightly reduce to prevent plastic look

    // Final color composition
    vec3 finalColor = uAmbientColor + diffuse + specular;

    // Gamma correction (optional, makes lighting look more natural)
...
`;
// Simple grid shader for container wireframe
...
let gravityMagnitude = parseFloat(gravitySlider.value);
...
const farValueLabel = document.getElementById('farValue');
let farPlane = parseFloat(farSlider.value);
...
// initial container scale from UI
// const containerScale declared above
...
let NUM_SPHERES = parseInt(particleCountSlider.value);
...
function regenerateParticles(count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    particles.push({ position: [modelMatrix[12], modelMatrix[13], modelMatrix[14]], velocity: [0, 0, 0], radius: 1.0 });
  }
}
// initial particles
regenerateParticles(NUM_SPHERES);
...
function animate(time) {
    ...
    gl.uniform4fv(gridProgramInfo.uniformLocations.color, [0.0, 1.0, 1.0, 0.7]);
    ...
    // Bind indices and draw triangles (grid shader will overlay lines)
    ...
    gl.uniform4fv(gridProgramInfo.uniformLocations.color, [0.0, 1.0, 1.0, 0.2]);
    gl.drawElements(gl.TRIANGLES, containerMesh.indexCount, gl.UNSIGNED_SHORT, 0);
    gl.disable(gl.BLEND);
    // Restore depth writes (if modified)
    gl.depthMask(true);
